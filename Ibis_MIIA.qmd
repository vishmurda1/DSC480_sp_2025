...
setwd("~/Dropbox/Projects/Senscio/Senscio_2024")

#df_pre <- read_csv("../data/2024-06-17T15-18_full_dataset.csv")  
df_pre <- read_csv("../data/9_9utilization_events.csv")
#df_pre <- read_csv("../data/2024-11-18_utilization_events.csv")


#df <- df_pre %>% mutate_at(vars(contains("date")), ~as_date(.x))
...
levels(df$event_type) <- c("emergency", "inpatient", "no_utilization", "observation")


### Set term = min number of days under observation,
### When term != 0, only count events in the first lag number of days.
## typically will just be term in that case
